...
async def get_all_emails(db: AsyncSession, category: Optional[str] = None) -> List[Email]:
    """Get all email records from database, optionally filtered by category"""
...
async def get_emails_by_category(category: str, db: AsyncSession) -> List[Email]:
    """Get email records by category"""
    statement = select(Email).where(func.upper(Email.category) == category.upper())
    result = await db.exec(statement)
    return result.all()
